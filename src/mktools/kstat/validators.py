...
class DateValidationReport:
    ...
    def valid_ratio(self) -> float:
        return 1.0 if self.total_rows == 0 else self.valid_rows / self.total_rows
...
class DateSeriesValidator:
    ...
    def validate(
        ...
        *,
        ...
        too_early_mask = normalized_dates < self.min_date if self.min_date is not None else pd.Series(False, index=series.index)
        too_late_mask = normalized_dates > self.max_date if self.max_date is not None else pd.Series(False, index=series.index)
        ...
        if report.invalid_rows:
            messages.append(f"Invalid date strings: {series[invalid_mask].drop_duplicates().astype(str).head(5).tolist()}")
        if report.future_rows:
            messages.append(f"Dates in the future: {series[future_mask].drop_duplicates().astype(str).head(5).tolist()}")
        if report.too_early_rows:
            messages.append(f"Dates before {self.min_date}: {series[too_early_mask].drop_duplicates().astype(str).head(5).tolist()}")
        if report.too_late_rows:
            messages.append(f"Dates after {self.max_date}: {series[too_late_mask].drop_duplicates().astype(str).head(5).tolist()}")
